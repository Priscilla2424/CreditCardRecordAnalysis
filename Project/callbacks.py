...
#         Input('submit-download', 'n_clicks'),
#         State('uploaded_table', 'data'),
#         State('uploaded_table', 'columns'),
#     )
#     def download_data(n_clicks, data, columns):
#         import io
#
#         if n_clicks:
#             if data and columns:
#                 df_download = pd.DataFrame(data, columns=[col['name'] for col in columns])
#                 buffer = io.StringIO()
#                 df_download.to_csv(buffer, index=False)
#                 buffer.seek(0)
#                 return dict(content=buffer.getvalue(), filename='download.csv')
#         return dash.no_update
#
#     # tab7.py callbacks
#     @app.callback(
#         Output('tab7_graph1', 'figure'),#         Input('submit_tab7_1', 'n_clicks'),
